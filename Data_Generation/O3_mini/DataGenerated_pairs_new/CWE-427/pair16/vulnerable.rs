...
impl ExecManager {
    fn new() -> ExecManager {
        // Here a value from an untrusted environment variable "ATTACKER_PATH" is used.
        // It is transformed into a CString via an unsafe block.
        // An attacker controlling "ATTACKER_PATH" can inject malicious search path elements.
        let c_path = unsafe {
            std::ffi::CString::new(
    ...
    fn spawn_process(&self) -> std::io::Result<()> {
        // The uncontrolled search path is passed to the spawned command.
        let mut cmd = Command::new("ls");
        cmd.env("PATH", &self.path);
        let status = cmd.status()?;
        // An unsafe block is used to simulate real-world pointer manipulation.
...
unsafe fn simulate_unsafe() {
    ...
    // Unsafe mutation through raw pointer.
    *pointer += 1;
}
...
fn main() {
    ...
    for _ in 0..2 {
        ...
        let handle = thread::spawn(move || {
            ...
            // Execute the process. If the unsafe PATH is exploited, this may fail.
