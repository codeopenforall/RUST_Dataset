//////////////////////////////////////////////////////////////
// Vulnerable Code Sample for Information Exposure (CWE-203) //
//////////////////////////////////////////////////////////////
...
impl Service {
    ...
    // Vulnerable: This function provides different error messages that leak if a user exists.
        ...
        for rec in data.iter() {
            if rec.username == name {
                if rec.pass_hash == input_hash {
                    ...
                    // Vulnerability: Reveals that the account exists by reporting a password error.
                    return Err("Password mismatch");
                }
            }
        }
        // Vulnerability: Distinct error reveals that the user does not exist.
        Err("User not found")
    }

    // A simple hash implementation using an unsafe block to iterate memory.
...
fn main() {
    ...
    // Spawn a thread to simulate concurrent authentication attempt.
