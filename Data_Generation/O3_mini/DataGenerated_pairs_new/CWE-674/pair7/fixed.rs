...
const MAX_DEPTH: usize = 1000; // Enforced recursion depth limit
...
impl Node {
    // Safe constructor used in the fixed code.
    fn new() -> Self {
...
impl Parser {
    ...
    // Parsing entry point that supplies an initial recursion depth of zero.
    fn parse_expr(&mut self) -> Result<Node, &'static str> {
        self.parse_nested(0)
    }

    // Recursive descent parser with an explicit recursion depth counter.
    fn parse_nested(&mut self, depth: usize) -> Result<Node, &'static str> {
        ...
        if self.input[self.pos] == '(' {
            ...
            while self.pos < self.input.len() && self.input[self.pos] == '(' {
                // Increment the recursion depth at each call.
