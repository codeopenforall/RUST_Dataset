...
impl Server {
    ...
    // In this version, the method first verifies the number of tasks
    // does not exceed a safe maximum (MAX_LIMIT).
    // If the 'count' is too high, it aborts before spawning any threads.
    // The unsafe block is still used for low-level operations,
    // but the amount of work is now properly bounded.
