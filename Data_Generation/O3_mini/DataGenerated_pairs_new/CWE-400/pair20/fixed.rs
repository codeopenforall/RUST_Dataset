use std::sync::{Arc, Mutex};
use std::thread;
use std::time::Duration;

#[derive(Debug)]
struct Connection {
    id: u64,
}

struct Service {
    connections: Arc<Mutex<Vec<Connection>>>,
    max_connections: usize,
}

/// This common interface defines how a resource‐managing object is constructed,
/// started, and queried.
pub trait ResourceManager {
    /// Creates a new instance using the provided limit to enforce resource cap.
    fn new(limit: usize) -> Self;
    /// Starts the resource acceptance loop.
    fn run(&self);
    /// Returns the current number of accepted resources.
    fn get_count(&self) -> usize;
}

impl ResourceManager for Service {
    fn new(limit: usize) -> Self {
        Service {
            connections: Arc::new(Mutex::new(Vec::new())),
            max_connections: limit,
        }
    }

    fn run(&self) {
        let connections = self.connections.clone();
        let max = self.max_connections;
        thread::spawn(move || {
            let mut id_generator = 0u64;
            loop {
                {
                    let mut cons = connections.lock().unwrap();
                    // Check against the maximum allowed to impose backpressure.
                    if cons.len() < max {
                        cons.push(Connection { id: id_generator });
                    }
                    // If the cap is reached, the connection is dropped.
                }
                id_generator = id_generator.wrapping_add(1);
                // Slight delay to avoid saturating the CPU.
                thread::sleep(Duration::from_millis(10));
            }
        });
    }

    fn get_count(&self) -> usize {
        self.connections.lock().unwrap().len()
    }
}

/// Alias used by external callers to create or test the resource manager.
/// This version enforces the connection limit.
pub type ResourceImpl = Service;

fn main() {
    let manager = ResourceImpl::new(1000);
    manager.run();
    // Prevent the program from exiting so the acceptance thread continues running.
    loop {
        thread::sleep(Duration::from_millis(500));
    }
}