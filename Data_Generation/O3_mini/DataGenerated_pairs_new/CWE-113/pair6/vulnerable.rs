...
impl ResponseGenerator for HttpResponder {
    fn build_response(&self, input: &str) -> String {
        // Build the response header using unsanitized input.
        // This unsafe block copies user input into a new buffer without any checks.
        let mut header = String::from("HTTP/1.1 200 OK\r\n");
        unsafe {
            // Directly obtaining a raw pointer to the input bytes.
...
fn main() {
    // Malicious input contains CRLF sequences to attempt HTTP header splitting.
    let input = "malicious\r\nInjected: Yes";
