...
impl HttpResponse {
    ...
    // Sanitizes input by stripping CR and LF characters.
    fn sanitize(input: &str) -> String {
        input.chars().filter(|&c| c != '\r' && c != '\n').collect()
    }

    // Sets a header after sanitizing the value.
    pub fn set_hdr(&mut self, name: &str, value: &str) {
        let safe_value = HttpResponse::sanitize(value);
        self.header = format!("{}: {}\r\n", name, safe_value);
        unsafe {
            // Even though unsafe is still used, the header no longer contains CRLF.
...
// Public function to generate a response header string; used by tests.
...
fn main() {
    // Use the same input to demonstrate that CRLF injection is prevented.
