//////////////////////////////////////////////
// Fixed Code - Correct Signature Verification
//////////////////////////////////////////////
use std::thread;

const DOMAIN: &[u8] = b"DOC"; // Domain separation constant
...
impl Record {
    // Now, compute the expected signature by also incorporating the domain.
    fn compute_expectation(&self, key: &[u8; 32]) -> [u8; 32] {
        let mut outcome = [0u8; 32];
        let bytes = self.content.as_bytes();
        // Incorporate DOMAIN into the computation for proper context.
        for i in 0..32 {
    ...
    // Safe verification using the properly computed expectation.
...
fn main() {
    ...
    // Simulate an attacker forging a record using the old flawed method.
    let content = String::from("Attack");
    let mut forged_signature = [0u8; 32];
    {
        let bytes = content.as_bytes();
        // Attacker forges signature without the required domain.
        for i in 0..32 {
