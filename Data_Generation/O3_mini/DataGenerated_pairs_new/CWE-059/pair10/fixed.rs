...
impl FileHandler {
    ...
    // Secure function: after resolving the file path, it canonicalizes the result
    // and verifies that it falls within the allowed directory.
    fn load_content(&self, filename: &str) -> Result<String, String> {
        let input_path = Path::new(filename);
        // Unsafe block kept only to mimic similar style, but soon replaced by safe canonicalization.
        ...
        // Canonicalize the final path so that it is expressed in absolute form.
        ...
        // Enforce that the canonical path lies inside the allowed directory.
        if !canonical.starts_with(&canonical_allowed) {
            return Err("Access denied: file outside allowed directory".into());
        }
        
        // Use concurrency to read the file (same pattern as before) but now on the validated path.
