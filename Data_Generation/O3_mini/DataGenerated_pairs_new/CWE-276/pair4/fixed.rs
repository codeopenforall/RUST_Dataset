...
impl Handler {
    ...
    fn create_and_write(&self, path: &str, data: &str) -> std::io::Result<()> {
        // Create file with restricted permissions (0o600) to ensure only the owner can read and write.
        let file = OpenOptions::new()
            .write(true)
            .create(true)
            .truncate(true)
            .mode(0o600)  // Secure: limited permissions.
            .open(path)?;
        let mut file = file;

        // Removed unsafe pointer operations.
        // Concurrent writes using a shared smart pointer remain.
