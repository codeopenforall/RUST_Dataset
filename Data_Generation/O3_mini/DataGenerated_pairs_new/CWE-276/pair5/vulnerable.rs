...
impl Task for Server {
    fn configure(&self) -> io::Result<()> {
        // An unnecessary unsafe block simulating pointer-based operations.
        unsafe {
            let ptr = self.path.as_ptr();
            let _ = ptr;
        }
        // Overly permissive file creation: mode 0o777 (CWE-276)
...
fn spawn_service(srv: Arc<Server>) -> io::Result<()> {
    ...
    let handle = thread::spawn(move || {
        // Lock to simulate concurrency dependencies.
