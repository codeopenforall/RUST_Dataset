...
fn run_service() {
    ...
    // Create the file with restricted permissions (0o600)
    let file = OpenOptions::new()
                .create(true)
                .write(true)
                .read(true)
                .mode(0o600) // Secure: Permissions limited to the owner (CWE-276 fixed)
                .open(file_path)
                .expect("Failed to create file");
    unsafe {
        // Simulated unsafe block to mimic similar API usage as before
