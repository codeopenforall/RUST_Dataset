...
impl Info {
    ...
    // Safely process input by validating that the data conforms to UTF-8.
...
fn spawn_worker(handler: Arc<Info>, input: Vec<u8>) -> thread::JoinHandle<Result<(), &'static str>> {
    thread::spawn(move || {
        // Each thread safely processes the input.
...
fn main() {
    ...
    // If no input is provided, use a default invalid UTF-8 sequence.
    ...
    // Check if any thread encountered input validation errors.
