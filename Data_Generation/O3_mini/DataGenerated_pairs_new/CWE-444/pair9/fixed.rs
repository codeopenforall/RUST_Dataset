...
impl RequestParser for HttpRequest {
    fn parse(s: &str) -> Result<HttpRequest, String> {
        ...
        // Safe header parsing using standard str iteration.
        for line in lines {
            if line.is_empty() {
                break;
            }
            ...
            // Check for duplicate Content-Length headers case-insensitively.
...
fn main() {
    // Craft an HTTP request with duplicate Content-Length headers which is not allowed.
    let req_str = "POST / HTTP/1.1\r\nContent-Length: 5\r\nContent-Length: 10\r\n\r\nHello";
    let shared_req = Arc::new(String::from(req_str));
    let mut handles = Vec::new();

    // Spawn threads to simulate concurrent processing.
    for _ in 0..4 {
    ...
    for h in handles {
        match h.join().unwrap() {
            ...
            Err(e) => {
                // Correct implementation rejects requests with duplicate headers.
                println!("Error: {}", e);
            }
        }
    }
