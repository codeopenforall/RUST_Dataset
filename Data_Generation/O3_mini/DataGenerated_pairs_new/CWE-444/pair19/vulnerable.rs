...
impl HttpRequest {
    // An unsafe helper to simulate low-level parsing by reading raw bytes.
    ...
    pub fn parse(raw: &str) -> Result<Self, String> {
        ...
        let body: String;
        // If both Transfer-Encoding and Content-Length are present, incorrectly use an unsafe offset.
        if transfer_encoding && content_length.is_some() {
            // Vulnerable: Improper pointer arithmetic can lead to inconsistent interpretation.
            unsafe {
                let ptr = body_str.as_ptr().offset(1); // Incorrectly offset by one byte.
                let len = content_length.unwrap();
                body = HttpRequest::parse_body(ptr, len);
            }
        } else if transfer_encoding {
            // Simulated chunked decoding: simply remove CRLF for this example.
...
fn process_request(raw: &str) -> Result<String, String> {
    // Using Arc/Mutex and a spawned thread to mimic concurrent processing.
...
fn main() {
    // This input uses both Transfer-Encoding and Content-Length headers.
