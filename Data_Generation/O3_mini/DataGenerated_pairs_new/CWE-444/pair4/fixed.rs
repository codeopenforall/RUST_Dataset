...
impl Message {
    fn new(raw: &str) -> Self {
        // Safely split the request into individual header lines.
        let headers = raw.split("\r\n").map(|s| s.to_string()).collect();
        Message { headers }
    }

    // Safely parses the "Content-Length:" header.
    // In case of multiple conflicting Content-Length values, it treats the request as invalid and returns 0.
            ...
            if let Some(rest) = line.strip_prefix("Content-Length:") {
                ...
                if let Ok(num) = token.parse::<u32>() {
                    if let Some(existing) = reported {
                        if existing != num {
                            // Detected conflicting Content-Length headers.
                            return 0;
                        }
                    } else {
                        reported = Some(num);
...
fn simulate(data: &str) -> u32 {
    // Simulate concurrent access similarly to the vulnerable version.
...
fn main() {
    // For a properly formed request, the header is parsed correctly.
