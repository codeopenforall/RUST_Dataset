/* Corrected implementation for HTTP request parsing that rejects ambiguous requests.
 * This version explicitly checks if both Content-Length and Transfer-Encoding headers are present.
 * It also uses safe concurrency primitives and avoids unsafe pointer conversions.
 */
...
fn process_request(input: &str) -> Result<Request, &'static str> {
    ...
    // Fixed: Reject requests that contain both Content-Length and Transfer-Encoding.
    ...
    if let Some(cl_val) = req.headers.get("Content-Length") {
        ...
        // Safely process chunked encoding without unsafe pointer arithmetic.
    ...
    // Safe concurrency: update the shared state using proper atomic operations without unsafe casts.
    use std::sync::Arc;
...
fn main() {
    // For the fixed version, provide a well-formed request that does not contain ambiguous headers.
    // Here we use only Content-Length.
    let request = concat!(
        "POST / HTTP/1.1\r\n",
        "Host: example.com\r\n",
        "Content-Length: 13\r\n",
