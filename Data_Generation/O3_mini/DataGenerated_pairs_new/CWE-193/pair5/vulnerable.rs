...
impl Container {
    ...
    // Processes an input byte by appending it unsafely to an internal buffer.
    // Contains an off-by-one error in pointer arithmetic.
    fn process(&self, value: u8) -> Result<(), &'static str> {
        ...
        unsafe {
            // Vulnerability: The pointer arithmetic has an off-by-one error.
            // Instead of writing at position "len", it writes at "len + 1".
...
fn main() {
    ...
    // Perform a series of sequential writes.
    for _ in 0..5 {
