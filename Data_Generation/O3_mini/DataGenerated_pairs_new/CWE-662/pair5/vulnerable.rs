...
fn increment_loop(shared: Arc<Mutex<Counter>>) {
    for _ in 0..1000 {
        ...
        let raw_ptr: *const Counter = &*guard; // Vulnerability: obtain raw pointer from locked data
        drop(guard); // Unlock too early
        unsafe {
            let counter_ref = &*raw_ptr; // Use raw pointer without protection
            counter_ref.bump();
        }
        thread::yield_now();
    }
