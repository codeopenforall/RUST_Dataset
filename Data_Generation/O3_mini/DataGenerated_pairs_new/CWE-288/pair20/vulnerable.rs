...
impl Handler {
    ...
    // Vulnerable authentication routine.
    // When debug_enabled is true, it bypasses all token checks.
    fn authenticate(&self, req: &Request) -> bool {
        // Authentication bypass if debug flag is enabled,
    ...
    fn serve_request(&self, req: Request) -> bool {
        ...
        let handle = thread::spawn(move || {
            let result = if debug {
                // Bypasses authentication if debug flag is active.
                true
            } else {
                unsafe {
...
fn main() {
    ...
    // Even though the request lacks an Authorization header,
    // the debug flag is active, which erroneously bypasses auth.
    let req = Request {
        headers: vec![
