...
fn process(input: &str) -> Vec<u8> {
    unsafe {
        // Incorrect computation: using the size of the reference,
        // not the actual number of bytes in the string.
        let size = mem::size_of::<&str>(); // Vulnerable: should be input.len()
