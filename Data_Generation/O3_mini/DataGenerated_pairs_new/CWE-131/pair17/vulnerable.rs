...
impl DataBuffer {
    fn new(input: &str) -> Self {
        unsafe {
            // FLAW: Incorrectly computing the allocation size using size_of_val on the input reference.
            // This returns the size of the String slice header (a constant) rather than the actual byte length.
            let size = mem::size_of_val(&input);
            let layout = Layout::from_size_align(size, 1).unwrap();
            let buf = alloc(layout);
            // Copies exactly 'size' bytes into the allocated memory.
