...
impl CustomBuffer {
    unsafe fn new(input: &str) -> CustomBuffer {
        // Incorrectly compute buffer size by using size_of_val on the reference.
        // This returns the size of the &str type (typically 16 bytes) rather than the actual string length.
        ...
        // Copy the input bytes into the allocated memory.
        // BUG: if input.len() > computed_size, this will overrun the allocated buffer.
    ...
    unsafe fn as_str(&self) -> &str {
        // Create a slice from the allocated memory using the (incorrect) allocated size.
...
impl Processor for ConcatProcessor {
    fn process(&self, text: &str) -> String {
        // Simulate concurrent work by splitting the text processing into two threads.
        let half = text.len() / 2;
...
fn main() {
    let raw_buffer;
    unsafe {
        // Using a string that exceeds the computed buffer size from size_of_val.
