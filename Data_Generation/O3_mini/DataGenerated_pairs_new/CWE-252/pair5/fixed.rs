...
impl Data {
    // This method performs an unsafe update on the shared counter.
    // It enforces the threshold check by returning an error if exceeded.
...
fn run() {
    ...
    let handle = thread::spawn(move || {
        unsafe {
            // FIX: Properly handling the Result from the update operation.
            match data_clone.update(150) {
                Ok(_) => {},
                Err(e) => {
                    // Failing fast if the update does not meet requirements.
