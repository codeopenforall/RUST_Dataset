...
impl fmt::Debug for Data {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // SECURE: Redacting the secret value to avoid leaking sensitive data.
        write!(f, "Data {{ id: {}, secret: <redacted> }}", self.id)
    }
}

fn process_data(data: Arc<Data>) {
    // Retain unsafe operations for complex legacy processing.
    unsafe {
        ...
        if secret_copy.contains("vulnerable") {
            // FIX: The debug representation no longer includes the raw secret.
